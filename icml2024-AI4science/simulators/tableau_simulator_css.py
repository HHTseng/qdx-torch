...
class TableauSimulatorCSS():
    ...
    def __init__(self,
        ...
        self.H_X = torch.zeros((self.n_H, self.n), dtype=torch.uint8) # shape (n_H, n)
        self.H_Z = torch.zeros((self.n - self.k - self.n_H, self.n), dtype=torch.uint8) # shape (n-k-n_H, n)

        for count,H_label in enumerate(init_H):
            self.H_X[count, H_label] = 1

        no_H_positions = set(range(k,n)) ^ set(init_H) # XOR between these two sets

        for count,no_H_label in enumerate(no_H_positions):
            self.H_Z[count, no_H_label] = 1
