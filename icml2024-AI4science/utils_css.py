...
class UtilsCSS():
    ...
    def CSS_error_operators(self, eval_d) -> torch.Tensor:
    ...
    def check_KL(self, E_mu):
        ...
        inS_X = torch.logical_xor(S_X[None, :, :], E_mu[:, None, :])
        inS_X = torch.logical_not(inS_X).all(dim=-1).to(torch.int32)

        inS_Z = torch.logical_xor(S_Z[None, :, :], E_mu[:, None, :])
        inS_Z = torch.logical_not(inS_Z).all(dim=-1).to(torch.int32)


        num_KL = 2*len(E_mu) - int(torch.sum(torch.any((E_mu @ self.tableau.H_X.T)%2, dim=1), dim=0)) - int(torch.sum(torch.any((E_mu @ self.tableau.H_Z.T)%2, dim=1), dim=0)) - int(torch.sum(inS_X)) - int(torch.sum(inS_Z))
