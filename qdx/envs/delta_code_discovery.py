...
class DeltaCodeDiscovery(Environment):
    """
    ...
    def step_env(
        ...
        max_reward = np.max(np.array([np.float32(0.), (reward - state.current_rew) / state.init_rew - state.x_t ], dtype=np.float32))

        delta_reward = np.float32((reward - state.current_rew) / state.init_rew)
