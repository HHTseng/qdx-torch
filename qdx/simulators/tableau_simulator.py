...
class TableauSimulator():
    ...
    def __init__(self,
        ...
        if initial_tableau is None:
            self.current_tableau = torch.eye(2*n).repeat(batch_size, 1, 1).to(torch.uint8) # Initialize the tableau with an empty circuit
        else:
            initial_tableau = torch.as_tensor(initial_tableau)
            self.current_tableau = initial_tableau.repeat(batch_size, 1, 1).to(torch.uint8) # Initialize the tableau with desired tableau
